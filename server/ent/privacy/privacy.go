...
func (f SongQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.SongQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.SongQuery", q)
}

// The SongMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type SongMutationRuleFunc func(context.Context, *ent.SongMutation) error

// EvalMutation calls f(ctx, m).
func (f SongMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.SongMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.SongMutation", m)
}
