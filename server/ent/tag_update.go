...
func (tu *TagUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if nodes := tu.mutation.RemovedSongsIDs(); len(nodes) > 0 {
	...
	if nodes := tu.mutation.SongsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.SongsTable,
...
func (tuo *TagUpdateOne) sqlSave(ctx context.Context) (t *Tag, err error) {
	...
	if nodes := tuo.mutation.RemovedSongsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.SongsTable,
	...
	if nodes := tuo.mutation.SongsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.SongsTable,
